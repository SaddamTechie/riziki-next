...
const VALID_DEPTS: Dept[] = ["men", "women", "beauty"];
...
export async function generateMetadata({
  ...
  const label = DEPT_LABELS[dept as Dept];
  if (!label) return {};
  return {
    title: `${label.heading} — Riziki`,
    description: `Shop the latest ${label.pronoun.toLowerCase()} fashion, accessories and more.`,
    openGraph: {
      title: `${label.heading} — Riziki`,
      description: `Shop the latest ${label.pronoun.toLowerCase()} fashion, accessories and more.`,
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-end justify-between">
          ...
          <Button variant="ghost" className="gap-1.5 text-sm" asChild>
            <Link href={`/${dept}/products?sort=newest`}>
              View all
    ...
    <section className="bg-secondary/30 py-12 sm:py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-end justify-between">
          ...
          <Button variant="ghost" className="gap-1.5 text-sm" asChild>
            <Link href={`/${dept}/products?sale=true`}>
              View all
              <ArrowRight className="h-4 w-4" />
            </Link>
          </Button>
...
export default async function DeptHomePage({
  params,
}: {
  params: Promise<{ dept: string }>;
}) {
      ...
      <section className="bg-primary py-10 text-primary-foreground">
        <div className="mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <Button variant="secondary" className="mt-4" asChild>
            <Link href={`/${dept}/products?sale=true`}>Shop the Sale</Link>
          </Button>
        </div>
      </section>
    </main>
